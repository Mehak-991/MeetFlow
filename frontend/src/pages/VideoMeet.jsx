...
export default function VideoMeetComponent() {
  ...
  const triggerFloatingEmoji = useCallback((emoji, senderName = "Guest") => {
  ...
  const connectToSocketServer = useCallback(() => {
    ...
    socketRef.current.on("connect", () => {
      ...
      console.log("[MeetFlow] Socket connected → join-call | meetingCode:", meetingCode, "| username:", username, "| userId:", userId);
      socketRef.current.emit("join-call", meetingCode, username, userId);
  ...
  }, [meetingCode, username, gotMessageFromServer, addMessage, triggerFloatingEmoji]);
...
const handleVideo = () => {
  ...
  const getPermissions = useCallback(async () => {
    try {
      ...
      let audioAvailableVal = false;
      try {
        const audioPermission = await navigator.mediaDevices.getUserMedia({
          audio: true,
        });
        if (audioPermission) {
          audioAvailableVal = true;
      ...
      if (videoAvailableVal || audioAvailableVal) {
        const userMediaStream = await navigator.mediaDevices.getUserMedia({
          video: videoAvailableVal,
          audio: audioAvailableVal,
        });
        if (userMediaStream) {
  ...
  // ── FIX: Extract meetingCode correctly from URL ────────────────────────────
  // Handles both /meeting/ABC123XYZ and /ABC123XYZ route patterns.
  // window.location.pathname.substring(1) gives "meeting/ABC123XYZ" for the
  // /meeting/:meetingCode route, which breaks Meeting.findOne({ meetingCode }).
  const rawPath = window.location.pathname;
  const meetingCode = rawPath.startsWith("/meeting/")
    ? rawPath.replace("/meeting/", "")   // strip leading "meeting/"
  ...
  useEffect(() => {
    const verifyMeeting = async () => {
      try {
        ...
        console.log("[MeetFlow] verifyMeeting → calling check-meeting/" + meetingCode);

        // ── FIX: Pass token so server can determine isHost correctly ──────────
        const res = await axios.get(`${server}/api/v1/users/check-meeting/${meetingCode}`, {
          params: token ? { token } : {}
        });
        const data = res.data;
        console.log("[MeetFlow] check-meeting response:", data);
        ...
        // ── FIX: Use server-side isHost (which compares hostId === userId) ───
        // Do NOT compare data.hostId === username (username ≠ MongoDB _id)
        ...
        console.error("[MeetFlow] Meeting verification failed:", err);
        // ── FIX: Show specific error messages instead of generic one ─────────
        if (err.response?.status === 404) {
          setMeetingError("Meeting not found. The invite link may be invalid or the meeting was deleted.");
        } else if (err.response?.status === 403) {
          setMeetingError("This meeting has been locked by the host.");
        } else if (err.response?.status === 410) {
          setIsExpired(true);
        } else {
          setMeetingError("Could not verify the meeting. Please check your connection and try again.");
        }
      } finally {
    ...
    <div className={styles.pageWrapper}>
      {askForUsername === true ? (
        ...
        <div className={styles.meetContainer} style={{ display: "flex", flexDirection: "row", overflow: "hidden" }}>
          ...
          >
            ...
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ opacity: 0.5 }}>|</span>
                <span style={{ cursor: "pointer", display: "inline-flex", alignItems: "center" }} title="Meeting details">â“˜</span>
              </div>
            ...
            {showEmojiPicker && (
              ...
              >
                {["â¤ï¸", "ðŸ‘", "ðŸŽ‰", "ðŸ‘", "ðŸ˜‚", "ðŸ˜®", "ðŸ˜¢", "ðŸ¤”", "ðŸ‘Ž", "ðŸ”¥", "ðŸš€", "ðŸ™Œ", "ðŸ‘ðŸ‘", "ðŸ˜Š"].map(emoji => (
                  <button
                    key={emoji}
            ...
            <div className={styles.controlBar} style={{
              ...
              >
                <span style={{ fontSize: "16px" }}>{isFullscreen ? "â›¶" : "â›¶"}</span>
              </IconButton>
              ...
              >
                ...
                <MenuItem onClick={() => { setModal(true); setActiveTab("assistant"); setMoreMenuAnchor(null); }} style={{ fontSize: "13px" }}>
                  <span style={{ marginRight: "12px" }}>ðŸ¤–</span> AI Assistant Sidepanel
                </MenuItem>
                <MenuItem onClick={() => { askAIQuestion("Summarize today's meeting."); setModal(true); setActiveTab("assistant"); setMoreMenuAnchor(null); }} style={{ fontSize: "13px" }}>
                  <span style={{ marginRight: "12px" }}>ðŸ“</span> Generate Meeting Summary
                </MenuItem>
                <MenuItem onClick={() => { askAIQuestion("What were today's decisions?"); setModal(true); setActiveTab("assistant"); setMoreMenuAnchor(null); }} style={{ fontSize: "13px" }}>
                  <span style={{ marginRight: "12px" }}>ðŸ“Œ</span> Extract Key Decisions
                </MenuItem>
                <MenuItem onClick={() => { downloadTranscript("txt"); setMoreMenuAnchor(null); }} style={{ fontSize: "13px" }}>
                  <span style={{ marginRight: "12px" }}>â¬‡</span> Download Transcript
                </MenuItem>
                ...
                <MenuItem onClick={() => { alert("Problem report submitted."); setMoreMenuAnchor(null); }} style={{ fontSize: "13px" }}>
                  <span style={{ marginRight: "12px" }}>ðŸž</span> Report a Problem
                </MenuItem>
                <MenuItem onClick={() => { alert("Help Center references opened."); setMoreMenuAnchor(null); }} style={{ fontSize: "13px" }}>
                  <span style={{ marginRight: "12px" }}>â“</span> Help Center
                </MenuItem>
              </Menu>
              ...
              >
                ...
                {audioDevices.map(d => (
                  <MenuItem key={d.deviceId} onClick={() => { setSelectedAudio(d.deviceId); handleDeviceChange(selectedVideo, d.deviceId); setMicMenuAnchor(null); }} style={{ fontSize: "13px" }}>
                    {selectedAudio === d.deviceId ? "âœ“ " : ""}{d.label || `Microphone ${d.deviceId.substring(0, 5)}`}
                  </MenuItem>
                ))}
              ...
              >
                ...
                {videoDevices.map(d => (
                  <MenuItem key={d.deviceId} onClick={() => { setSelectedVideo(d.deviceId); handleDeviceChange(d.deviceId, selectedAudio); setCamMenuAnchor(null); }} style={{ fontSize: "13px" }}>
                    {selectedVideo === d.deviceId ? "âœ“ " : ""}{d.label || `Camera ${d.deviceId.substring(0, 5)}`}
                  </MenuItem>
                ))}
                <Divider sx={{ borderColor: "rgba(255,255,255,0.1)" }} />
          ...
          {/* CHAT PANEL â€” animated, RIGHT side */}
          <AnimatePresence>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", backgroundColor: "rgba(255,255,255,0.05)", borderRadius: "24px", padding: "4px 12px", border: "1px solid rgba(255,255,255,0.1)" }}>
                  <TextField
                    ...
                    fullWidth
                  />
                  <IconButton onClick={sendMessage} disabled={isChatDisabled && !isHost} size="small" style={{ color: "#fff" }}>âž¤</IconButton>
                </div>
              </motion.aside>
            )}
          </AnimatePresence>

          {/* AI ASSISTANT PANEL â€” animated, RIGHT side */}
          <AnimatePresence>
              ...
              >
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                  <span style={{ fontSize: "16px", fontWeight: "600", color: "#fff", display: "flex", alignItems: "center", gap: "8px" }}>
                    <span>ðŸ¤–</span> AI Meeting Assistant
                  </span>
                  <button
          ...
          {/* PEOPLE PANEL â€” animated, RIGHT side */}
          <PeopleSidebar
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "center", borderBottom: "1.5px solid #1a73e8", paddingBottom: "12px", marginBottom: "20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", color: "#1a73e8", fontWeight: "600", fontSize: "14px" }}>
            <span style={{ fontSize: "18px" }}>ðŸ‘¤+</span> Invite
          </div>
        </div>
